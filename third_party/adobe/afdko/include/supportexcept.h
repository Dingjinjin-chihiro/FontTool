...
/* Font-tools extraction: C++ exception mode is intentionally removed.
 * Embedded AFDKO always uses its original C setjmp/longjmp containment. */
#ifdef USE_CXX_EXCEPTION
#undef USE_CXX_EXCEPTION
#endif
...
typedef struct _t_Exc_buf {
    ...
    int Code;        /* Internal AFDKO status */
} _Exc_Buf;
