...
/*
 * PoDoFo FontTools integration note:
 * The retained AFDKO font conversion/subset path does not parse XML.
 * ctlStreamCallbacks keeps xmlDocPtr only for source compatibility with
 * upstream AFDKO. A pointer to an incomplete type is sufficient here, so the
 * font-tools integration does not require libxml2 headers or libraries.
 */
